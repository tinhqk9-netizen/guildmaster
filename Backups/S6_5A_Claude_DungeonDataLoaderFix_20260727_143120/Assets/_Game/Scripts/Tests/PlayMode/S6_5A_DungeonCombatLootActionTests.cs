...
namespace GuildMaster.Tests.PlayMode
{
    ...
    public class S6_5A_DungeonCombatLootActionTests
    {
        ...
        public IEnumerator DungeonRun_Fights_Kills_Loots_Collects_AndPersists()
        {
            ...
            // --- 1. a real adventurer -------------------------------------------------------
            CharacterRuntime hero = services.Character.GetAllCharacters().FirstOrDefault();
            if (hero == null)
            {
                AdventurerDefinition advDef = services.Database.GetAll<AdventurerDefinition>().FirstOrDefault();
                Assert.IsNotNull(advDef, "no AdventurerDefinition loaded");
                hero = services.Character.CreateCharacter(advDef.id);
            }
