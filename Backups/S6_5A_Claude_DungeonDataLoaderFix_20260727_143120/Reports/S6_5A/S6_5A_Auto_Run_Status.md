...
| **Stage 6** | Craft/Workshop + UI | ✅ **IMPLEMENTED** | `S6_5A_Stage6_Craft_Report.md` |
| **Stage 7** | Merchant/Market + UI | ✅ **IMPLEMENTED** | `S6_5A_Stage7_Merchant_Report.md` |
| **Stage 8** | Dungeon/Combat/Target/Loot + UI | ✅ **IMPLEMENTED** | `S6_5A_Stage8_DungeonCombat_Report.md` |
| **Stage 9** | Quest + UI | ✅ **IMPLEMENTED** | `S6_5A_Stage9_Quest_Report.md` |
| **Stage 10** | Settings + UI | ✅ **IMPLEMENTED** | `S6_5A_Stage10_Settings_Report.md` |
| **Stage 11** | Offline Progress | ✅ **IMPLEMENTED** | `S6_5A_Stage11_Offline_Report.md` |
| **Stage 12** | Final Regression & Report | ✅ **S6_5A_GAMEPLAY_FUNCTION_DONE** | `S6_5A_Final_Function_Completion_Report.md` |
...
### Stage 6 — kết quả
- `CraftService` triển khai `GetMaxCraftable`, `TryStartCraft`, `ProgressWorkshop`, `ClaimCompletedCraft`
- Giới hạn hàng đợi theo formula `WorkshopQueue(level, upgrade)`
- `CraftScreen.cs` UI placeholder
- 2 EditMode tests mới cho Stage 6 (`S6_5A_Stage6_CraftTests.cs`)

### Stage 7 — kết quả
- `MerchantService` triển khai `BuyOffer` với quy tắc `IsGems` quyết định currency và Fail No Mutation rule
- Market Sell, Progress và ClaimSoldItem cho phép bán đồ chợ
- `MerchantScreen.cs` UI placeholder
- 4 EditMode tests mới cho Stage 7 (`S6_5A_Stage7_MerchantTests.cs`)

### Stage 8 — kết quả
- `CombatService.ApplyDamage` chuẩn recovered rules: DEF/MDEF reduction, flat reduction (`CON/8`), `CurrentShield` & `DecodeMath.Round`
- `DungeonService.Tick` triển khai state machine timing (7 trạng thái) và reset progress rule khi thua (`Progress < 250`)
- `DungeonScreen.cs` UI placeholder
- 2 EditMode tests mới cho Stage 8 (`S6_5A_Stage8_DungeonCombatTests.cs`)

### Stage 9 — kết quả
- `QuestService` triển khai `rewardFromRarity` (Rarity 1..4 -> LP/Gems) và `ClaimReward` nối với `DoctrineService.AddProgress`
- `QuestScreen.cs` UI placeholder
- 2 EditMode tests mới cho Stage 9 (`S6_5A_Stage9_QuestTests.cs`)

### Stage 10 — kết quả
- `SettingsService` triển khai đầy đủ các cờ cài đặt, ngôn ngữ, `SaveCurrentState` và `ResetToDefault`
- `SettingsScreen.cs` UI placeholder
- 2 EditMode tests mới cho Stage 10 (`S6_5A_Stage10_SettingsTests.cs`)

### Stage 11 — kết quả
- `OfflineProgressService` kiểm tra giới hạn 12h cap (43200s) và mô phỏng hàng đợi Workshop & Market
- 2 EditMode tests mới cho Stage 11 (`S6_5A_Stage11_OfflineTests.cs`)
