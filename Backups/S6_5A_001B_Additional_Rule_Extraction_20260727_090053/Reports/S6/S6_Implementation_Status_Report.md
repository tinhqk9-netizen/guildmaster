...
# Post-S6 Addendum 2026-07-25 (18:03) — Editable Scene UI Workflow

## Post-S6 workflow issue: Runtime-generated UI not editable before Play
User báo: mở `Main.unity`/`Boot.unity` khi chưa Play thì không thấy đủ UI panel để tự chỉnh, nghi ngờ do UI được tạo bằng runtime code.

**Kết quả audit — chẩn đoán ban đầu cần điều chỉnh:** UI **không** hề được tạo runtime. Toàn bộ **66 GameObject đã persist đầy đủ** trong `Main.unity`; `UIRuntimeBootstrap` chỉ `FindObjectsByType`/`FindFirstObjectByType` để bind service vào object có sẵn, **không có `new GameObject()`/`Instantiate()` nào**. Nguyên nhân thật: 7 screen (`Inventory`, `Character`, `Dungeon`, `Craft`, `Merchant`, `Settings`, `Popup`) bị `UIWiringGenerator` đặt `m_IsActive: 0` tại edit-time → vẫn nằm trong Hierarchy (chữ xám mờ) nhưng không render trong Scene view.

## Resolution: Editable Main scene UI baked/persisted
Vì scene đã đầy đủ, giải pháp rủi ro thấp nhất là **không sửa `Main.unity`, không sửa `UIRuntimeBootstrap`, không sửa generator** — chỉ bổ sung Editor tool:

- **File mới:** `Assets/_Game/Scripts/Editor/UIScreenPreviewTool.cs` (editor-only, bọc `#if UNITY_EDITOR`)
- **8 menu item:** `GuildMaster → UI → Preview: Show All Screens` / `Reset (Hide All Screens)` / `Only Inventory` / `Only Character` / `Only Dungeon` / `Only Craft` / `Only Merchant` / `Only Settings`
- **An toàn tuyệt đối với runtime:** `UIService.RegisterScreen()` gọi `Hide()` ngay khi đăng ký → mỗi lần Play runtime tự chuẩn hoá lại trạng thái active của mọi screen, bất kể scene file để bật hay tắt.
- Tool **chỉ toggle `SetActive`** — không tạo, không xoá, không rewire gì.

**Báo cáo chi tiết:** `Reports/S6_5/EditableSceneUI_Audit_Report.md`, `Reports/S6_5/EditableSceneUI_Implementation_Report.md`, `Reports/S6_5/Pre_S6_5_EditableSceneUI_Backup_Report.md`.
**Decision:** `EDITABLE_SCENE_UI_DONE_NEEDS_USER_PLAYTEST` (cần 1 lần reimport + Play test xác nhận compile).

## New next step
**S6.5 Visual Completion / Asset Finalization** — dùng chính các scene object editable đã xác nhận ở trên (HUD, 6 nav button, 7 screen, Back button) làm nền để hoàn thiện visual. **Chưa bắt đầu trong phiên này**: không generate asset, không dùng Higgsfield.
