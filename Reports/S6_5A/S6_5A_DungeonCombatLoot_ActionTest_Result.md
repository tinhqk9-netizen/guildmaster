...
| Enemy spawned | at least one enemy in the encounter | yes (hp before 60) | PASS |
| Damage applied | enemy HP decreases | hp 60 -> 1 | PASS |
| Enemy death | at least one enemy reaches 0 HP | yes | PASS |
| Loot in area chest | PendingDrops non-empty | 1 entries / 1 items after 270 ticks | PASS |
| Loot bypass check | chest loot is NOT in storage yet | inventory[beast_pelt] = 0 | PASS |
| CollectDrops | chest empties into storage | transferred=1, inventory[beast_pelt] 0 -> 1 | PASS |
| Dungeon progress | progress advanced during the run | progress=1 | PASS |
| Save/reload inventory | looted item still present (beast_pelt) | 1 | PASS |
| Save/reload progress | progress preserved (1) | 1 | PASS |
| OVERALL | damage + death + chest loot + collect + persist | all verified in 270 ticks | PASS |
