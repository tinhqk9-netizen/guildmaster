...
| Loot in area chest | PendingDrops non-empty | 1 entries / 1 items after 215 ticks | PASS |
| Loot bypass check | chest loot is NOT in storage yet | inventory[boar_tusk] = 0 | PASS |
| CollectDrops | chest empties into storage | transferred=1, inventory[boar_tusk] 0 -> 1 | PASS |
| Dungeon progress | progress advanced during the run | progress=0 | **FAIL** |
| Save/reload inventory | looted item still present (boar_tusk) | 1 | PASS |
| Save/reload progress | progress preserved (0) | 0 | PASS |
| OVERALL | damage + death + chest loot + collect + persist | all verified in 215 ticks | PASS |
