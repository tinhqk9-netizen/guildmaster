...
## 6. Decode Logic Coverage Audit Status (bổ sung 2026-07-25, phase S6.5A audit)
Đã hoàn tất audit đối chiếu decode ↔ Unity. Báo cáo: `Reports/S6_5A/` (6 file).

**Kết quả trả lời câu hỏi "đã lấy đủ logic từ decode chưa?": ❌ CHƯA ĐỦ.**

| Đối chiếu | Decode | Unity | Tỷ lệ |
|---|---|---|---|
| Formula (`Formulas.java`) | 20 method | 6 + 1 stub rỗng | **30%** |
| Save state (`Data.java`) | 140 field | 17 field | **12%** |
| Combat/Dungeon (`Area.java`) | 3164 dòng | 341 dòng | **~11%** |
| Quest (`QuestsManager.java`) | 468 dòng | 87 dòng | **19%** |
| Service wired runtime | — | 3/14 | **21%** |
| Player action mutate được state | — | **0** | **0%** |

**9 gap Critical**, trong đó 3 gap chặn cứng gameplay:
- **G-B02**: `CalculateDamage_ManualPortRequired()` là **stub rỗng** → chưa có công thức damage nào
- **G-B03**: `CharacterService.GetTotalStat()` **hardcode `levelMultiplier = 1.0f`** → chỉ số nhân vật hiện SAI
- **G-B14**: **Tavern chưa port** → không có adventurer → chặn cả chuỗi (không party → không combat → không loot → inventory rỗng → money = 0)

**Chưa port gì:** Doctrine (8 instance + 16 save field), Tavern/Quarters/Shelter, Pet (data có 21), Raid (data có 12), Achievements/IAP.

**Quan trọng:** chỉ **5/43 gap** là asset-only và **không gap nào trong đó chặn chức năng** → **thiếu asset KHÔNG phải nguyên nhân game chưa chơi được**.

Decision phase audit: `S6_5A_AUDIT_FOUND_CRITICAL_MISSING_LOGIC`

## 7. Function Completion Required Before Asset Finalization
Kế hoạch 13 task: `Reports/S6_5A/S6_5A_Function_Completion_Plan.md`. **12/13 task chỉ cần placeholder asset hoặc không cần asset** → không task nào bị chặn bởi thiếu asset.

Bắt đầu bằng **S6.5A-001 Rule Extraction** (chỉ đọc decode + viết tài liệu rule, không code) vì 6 task sau phụ thuộc vào nó.

## 8. Next Phase
- **S6.5A — Function Completion with Placeholder Assets** ← phase tiếp theo
- **S6.5B — Visual/Asset Finalization (Claude + MCP Higgsfield)**: **chỉ làm sau khi function gap đã rõ và S6.5A xong.** Làm đẹp UI trước sẽ phải dựng lại khi thêm chức năng.

**Cần user quyết định 2 điểm scope:** (1) Raid có thuộc scope rebuild không? (2) Achievements/IAP có thuộc scope không?
